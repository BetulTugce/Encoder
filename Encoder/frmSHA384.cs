...
namespace Encoder
{
    public partial class frmSHA384 : Form
    {
        ...
        private void btnSifrele_Click(object sender, EventArgs e)
        {
            try
            {
                txtCikti.Text = SHA384Sifrele(txtGirdi.Text);
            }
            catch
            {
        ...
        static string SHA384Sifrele(string text)
        {
            SHA384 sha384Encrypting = new SHA384CryptoServiceProvider();
            byte[] bytes = sha384Encrypting.ComputeHash(Encoding.UTF8.GetBytes(text));
